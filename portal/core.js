...
////////////////////////////////////////////////////insert , replace , remove , get , set  
		...
		getAttribute : function(node ,attr){
			var fixAttr = fmes.Dom.attrFixMap[attr];
			if(fixAttr){
				return node[attr];
			}else{
		...
		setAttribute : function(node , attributeName , attributeValue){
			//2 parameter (node , {})
			var attrFixMap = fmes.Dom.attrFixMap;
		...
		removeAttribute : function(node , attr){
			attr = fmes.Dom.attrFixMap[attr] || attr;
			var fixAttr = fmes.Dom.attrFixMap[attr];
			if(fixAttr){
				delete node[fixAttr];
			}else{
		...
		getDocumentWidth : function(){
			var width = 0 , view_width = fmes.Dom.getViewportWidth();
				// Mozilla   
 		...
 		getDocumentHeight : function(){
			var height = 0 , view_height = fmes.Dom.getViewportHeight();
		...
		isMouseIn : function(target , event){
			var event = fmes.Event.fixEvent(event),
				targetPosition = fmes.Css.getGlobalPosition(target),
				targetSize = { width : fmes.$(target).width(),
							   height : fmes.$(target).height()
							   };
	...
	fmes.Css = {
		...
		*/
		setStyle : function(target , css , cssVal){
			...
			// target, "cssName:value;cssName2:value"  
			var camelize = fmes.String.camelize;
			if(arguments.length == 2 && typeof arguments[1] == "string"){
				...
				for(var property in arguments[1]){
					var _property = fmes.String.trim(camelize(property));
				...
				var property = fmes.String.trim(camelize(arguments[1]));
				if(property == 'opacity'){
		...
		setWidth : function(target , value){
			if(fmes.Util.isNumber(value) ||  value - 0 )
			{
	...
	fmes.Class = {
		...
		define : function( namespace , config){
			...
			config.singleton = config.singleton && fmes.String.trim(config.singleton.toString().toLowerCase()) == 'true' ? true : false;
			...
			if(config.extend){
				if( typeof config.extend === 'string'){
					...
					if(extend){
						...
						throw new Error('fmes.Class.define: extend from a invalid Class:[' + config.extend + ']');
					}
				}else{
		...
		addMethods : function(_class){
			var arguNums = arguments.length,
				isOverride = false;
			if( fmes.Util.isBoolean(arguments[arguNums-1]) )
			{	
	...
	fmes.widget = {};
	fmes.widget.create = function(config){
		if(config.xType){
			//console.log(config);
			var widget = new fmes.widget[fmes.String.capitalize(config.xType)](config);
			return widget;
		}else{
			//console.log(config);
			throw new Error("create type(xtype) is undefine in config arguments.");
		}
	};
	/*
	 *json.js
...
(function(){
	...
	fmes.$.fn = function(selector, context){
		...
		if(Util.isString(selector)){
			this.elements = fmes.Selector(selector);
	...
	fmes.Class.addMethods(fmes.$.fn  ,{
			get : function(index){
					var Util = fmes.Util;
					if(Util.isNumber(index) && this.elements[index]){
	...
	fmes.Class.addMethods(fmes.$.fn  ,{
			...
			attr : function(){
				var _arguments = arguments , len = _arguments.length,
					_getAttribute = fmes.Dom.getAttribute,
					_setAttribute = fmes.Dom.setAttribute;

				if(len == 1 ){
					var argu = _arguments[0];
					if(typeof argu === 'string'){
						return _getAttribute(this.elements[0] , argu);
					}else if(typeof argu === 'object'){
						this.each(function(i,elem){
							_setAttribute(elem,argu);
						});
					}
				}else if(len == 2 && typeof _arguments[0] == 'string' && typeof _arguments[1] == 'string'){
					this.each(function(i,elem){
						_setAttribute(elem, _arguments[0] , _arguments[1]);
					});
				}
				return this;				
			},
